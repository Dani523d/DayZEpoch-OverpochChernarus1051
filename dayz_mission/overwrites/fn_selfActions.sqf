...
if ("ItemToolbox" in _weapons) then {
    ...
    if((speed player <= 1) && hasBikeItem && _canDo) then {
        if (s_player_deploybike < 0) then {
            s_player_deploybike = player addaction[("<t color=""#007ab7"">" + ("Deploy Bike") +"</t>"),"addons\bike\deploy.sqf","",5,false,true,"", ""];
        };
    } else {
...
if (s_player_deploybike2 < 0) then {
        s_player_deploybike2 = player addaction[("<t color=""#007ab7"">" + ("Re-Pack Bike") +"</t>"),"addons\bike\pack.sqf","",5,false,true,"", ""];
    };
} else {
