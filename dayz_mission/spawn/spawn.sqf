private ["_counter","_debug","_donorBase","_donorUID","_dragee","_findSpot","_isNear","_isZero","_nearPlayers","_pos","_position","_seldLoc","_seldRandom","_unit","_waitScript"];

_debug = [-7245.377,19535.367,0];
waitUntil {!isNil "dayzPlayerLogin2"};
waitUntil {count dayzPlayerLogin2 > 0};
dayzPlayerLogin2 set [0,[0,_debug]];
waitUntil {!isNil "dayz_gui"};
player setPosATL _debug;
cutText ["","BLACK OUT",0];
...
hidePlayers = [];
_dragee = _debug nearEntities ["Survivor1_DZ",10];
...
if (spawnSelect == 40) exitWith {
	...
	//### BEGIN MODIFIED CODE: ebay spawn height - change 2000 to 1000
	if (haloSelect == 1) then {_pos = [_position select 0,_position select 1,1000];player setPosATL _pos;player setVariable["lastPos",_pos,true];[player,1000] spawn BIS_fnc_halo;} else {player setPosATL _position;};
...
while {_findSpot and _counter < 20} do {
	...
	{if ((!isNull _x) and (getPlayerUID _x != "")) then {_isNear = true;};} count _nearPlayers;
